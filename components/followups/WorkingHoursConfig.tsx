...
export function WorkingHoursConfig({ config, onChange }: WorkingHoursConfigProps) {
  ...
  const validateConfig = () => {
    ...
    if (!timeRegex.test(config.start)) {
      newErrors.push("L'heure de début doit être au format HH:mm (ex: 07:00)");
    }
    if (!timeRegex.test(config.end)) {
      newErrors.push("L'heure de fin doit être au format HH:mm (ex: 18:00)");
    }

    // Validate time logic
    if (config.start >= config.end) {
      newErrors.push("L'heure de fin doit être postérieure à l'heure de début");
    }
  ...
  const handleWorkingDayToggle = (day: string, checked: boolean) => {
    ...
    updateConfig({ working_days: newDays as any });
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Sélectionnez le fuseau horaire de référence pour les heures ouvrables
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Définissez la plage horaire pendant laquelle les relances peuvent être envoyées
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Sélectionnez les jours de la semaine pendant lesquels les relances peuvent être envoyées
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                ...
                <div className="text-sm text-green-700 mt-1">Fuseau horaire</div>
              </div>
            </div>
            ...
            <div className="text-center">
              <div className="text-sm text-green-700">
                <strong>Plage horaire :</strong> {config.start} - {config.end} ({summary.timezone})
              </div>
      ...
      <Alert>
        ...
        <AlertDescription>
          Les relances seront automatiquement programmées uniquement pendant les heures et jours ouvrables définis.
          Si une relance est programmée en dehors de ces créneaux, elle sera reportée au prochain créneau disponible.
        </AlertDescription>
      </Alert>
    </div>
  );

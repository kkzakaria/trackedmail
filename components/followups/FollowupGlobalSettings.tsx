...
export function FollowupGlobalSettings({
  settings,
  onChange,
}: FollowupGlobalSettingsProps) {
  ...
  const validateSettings = useCallback(() => {
    ...
    if (
      ...
      settings.default_interval_hours > 720
    ) {
      newErrors.push(
        "L&apos;intervalle par défaut doit être entre 1 heure et 30 jours"
      );
    }

    if (settings.stop_after_days < 1 || settings.stop_after_days > 365) {
      newErrors.push(
        "La durée d&apos;expiration doit être entre 1 et 365 jours"
      );
    }
    ...
    if (
      ...
      newErrors.push(
        "La limite de taux doit être entre 1 et 1000 emails par heure"
      );
    }
  ...
  const getEfficiencyScore = () => {
    ...
    else if (settings.max_followups === 2 || settings.max_followups === 4)
      score += 20;
    else if (settings.max_followups === 1 || settings.max_followups === 5)
      score += 15;
    else score += 10;

    // Interval (optimal: 72-120 hours)
    if (
      settings.default_interval_hours >= 72 &&
      settings.default_interval_hours <= 120
    )
      score += 25;
    else if (
      settings.default_interval_hours >= 48 &&
      settings.default_interval_hours <= 168
    )
      score += 20;
    else score += 10;

    // Stop after days (optimal: 14-30 days)
    if (settings.stop_after_days >= 14 && settings.stop_after_days <= 30)
      score += 25;
    else if (settings.stop_after_days >= 7 && settings.stop_after_days <= 60)
      score += 20;
    else score += 10;
    ...
    <div className="space-y-6">
      ...
      >
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <Badge
              variant={settings.system_enabled ? "default" : "destructive"}
            >
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="font-medium">
                {settings.system_enabled
                  ? "Système de relances activé"
                  : "Système de relances désactivé"}
              </div>
            ...
            <Switch
              ...
              onCheckedChange={checked =>
                updateSettings({ system_enabled: checked })
              }
            />
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                <Label htmlFor="max-followups" className="min-w-[60px]">
                  Maximum
                </Label>
                <Input
                  id="max-followups"
              ...
              <div className="grid grid-cols-5 gap-1">
                {[1, 2, 3, 4, 5].map(num => (
                  <Button
                    key={num}
                    variant={
                      settings.max_followups === num ? "default" : "outline"
                    }
                    size="sm"
              ...
              <Alert>
                ...
                <AlertDescription className="text-sm">
                  Recommandé : 3 relances (compromis optimal entre efficacité et
                  respect)
                </AlertDescription>
              </Alert>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                <Label htmlFor="interval-hours" className="min-w-[60px]">
                  Heures
                </Label>
                <Input
                  id="interval-hours"
                  ...
                  onChange={e =>
                    updateSettings({
                      default_interval_hours: parseInt(e.target.value) || 96,
                    })
                  }
                  className="w-20 text-center"
                />
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    updateSettings({ default_interval_hours: 168 })
                  }
                >
                  1 semaine
                </Button>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                <Label htmlFor="stop-days" className="min-w-[60px]">
                  Jours
                </Label>
                <Input
                  id="stop-days"
                  ...
                  onChange={e =>
                    updateSettings({
                      stop_after_days: parseInt(e.target.value) || 30,
                    })
                  }
                  className="w-20 text-center"
                />
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center space-x-4">
                <Label htmlFor="rate-limit" className="min-w-[60px]">
                  Limite
                </Label>
                <Input
                  id="rate-limit"
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <div
                  className={`text-3xl font-bold ${getScoreColor(efficiencyScore)}`}
                >
                  {efficiencyScore}%
                </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-center md:grid-cols-4">
              ...
              <div>
                <div className="text-lg font-semibold">
                  {settings.stop_after_days}j
                </div>
                <div className="text-sm text-gray-600">Expiration</div>
              </div>
              <div>
                <div className="text-lg font-semibold">
                  {settings.rate_limit_per_hour}/h
                </div>
      ...
      {errors.length === 0 && (
        <Alert>
          ...
          <AlertDescription>
            Configuration valide. Les paramètres seront appliqués à toutes les
            nouvelles relances.
          </AlertDescription>
        </Alert>
      )}

...
import { useMailboxes, useDeleteMailbox, useToggleMailboxStatus, useSyncMailbox } from '@/lib/hooks/use-mailboxes';
import { useAuth } from '@/lib/hooks/use-auth';
import { Eye, Edit, Trash2, Power, RefreshCw, Search, Plus } from 'lucide-react';
...
export function MailboxList({ onCreateNew }: MailboxListProps) {
  ...
  const handleDelete = async (id: string, email: string) => {
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer la boîte mail ${email} ?`)) {
      try {
    ...
    <div className="space-y-4">
      ...
      {!isLoading && (
        <p className="text-sm text-muted-foreground">
          {mailboxData?.count} boîte{(mailboxData?.count ?? 0) > 1 ? 's' : ''} mail trouvée{(mailboxData?.count ?? 0) > 1 ? 's' : ''}
        </p>
      )}
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      {mailbox.last_sync ? (
                        <span>
                          Dernière sync: {new Date(mailbox.last_sync).toLocaleDateString('fr-FR', {
                            day: '2-digit',
                            month: '2-digit',
                            year: 'numeric',
                            hour: '2-digit',
                      ...
                      {mailbox.microsoft_user_id && (
                        <span>ID Microsoft: {mailbox.microsoft_user_id.slice(0, 8)}...</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {canManageMailboxes && (
                      <>
                        ...
                        >
                          <RefreshCw className={`h-4 w-4 ${syncMailboxMutation.isPending ? 'animate-spin' : ''}`} />
                        </Button>
